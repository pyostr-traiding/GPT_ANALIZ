...
def handle_general_script(
    ...
    prompt = get_prompt(prompt_code='zone_analiz_final')
    if not prompt:
        add_message(chat_uuid, 'general_analysis', 'text', 'Ошибка получения промпта', role='system', code=data.extra.code, context=data.extra.context)
        return
    prompt = f'Данные зон:{str(report_zones)} \nДанные тренда:{str(data_trend)} \n{prompt.prompt}'
    gpt_final = gpt_step(chat_uuid, prompt, message, 'финальный отчет', data=data)
    if not gpt_final:
        return True
